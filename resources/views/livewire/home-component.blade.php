    ...
    <div class="container">
                ...
                @foreach ($sliders as $slide)
                    <div class="item-slide">
                        ...
                        <div class="slide-info slide-1">
                            ...
                            <p class="sale-info">À seulement: <span class="price">{{ $slide->getFormatedPrice() }}</span></p>
        ...
        @if ($sale_products->count() > 0)
            <div class="wrap-show-advance-info-box style-1 has-countdown">
                <h3 class="title-box">Articles en Promo !</h3>
                    ...
                    @foreach ($sale_products as $product)
                        <div class="product product-style-2 equal-elem ">
                            <div class="product-thumnail">
                                ...
                                <div class="wrap-btn">
                                    <a href="{{ route('product.details', ['slug' => $product->slug]) }}" class="function-link">Voir détails</a>
                                </div>
                            </div>
                            <div class="product-info">
                                <a href="{{ route('product.details', ['slug' => $product->slug]) }}" class="product-name"><span>{{ $product->name }}</span></a>
                                <div class="wrap-price"><span class="product-price"></span></div>
                                <div class="wrap-price"><ins><p class="product-price">{{ number_format($product->sale_price, 0, '', ' ') }} Fcfa</p></ins> <del><p class="product-price">{{ number_format($product->regular_price, 0, '', ' ') }} Fcfa</p></del></div>
                            </div>
                        </div>
                    @endforeach
        ...
        <div class="wrap-show-advance-info-box style-1">
            <h3 class="title-box">Articles récemment ajouté</h3>
        ...
        <!--Product Categories-->
        <div class="wrap-show-advance-info-box style-1">
            <h3 class="title-box">Produits par catégorie</h3>
            <div class="wrap-top-banner">
                <a href="#" class="link-banner banner-effect-2">
                    ...
                    <div class="tab-contents">
                        @foreach ($categories as $key => $category)
                            <div class="tab-content-item {{ $key == 0 ? 'active' : '' }}" id="category_{{ $category->id }}">
                                <div class="wrap-products slide-carousel owl-carousel style-nav-1 equal-container" data-items="5" data-loop="false" data-nav="true" data-dots="false" data-responsive='{"0":{"items":"1"},"480":{"items":"2"},"768":{"items":"3"},"992":{"items":"4"},"1200":{"items":"5"}}' >
                                    @php
                                        $c_products = DB::table('products')->where('category_id', $category->id)->get()->take($nbr_of_prodiucts);
                                    @endphp
                                    @foreach ($c_products as $product)
                                        <div class="product product-style-2 equal-elem ">
                                            ...
                                            <div class="product-info">
                                                <a href="{{ route('product.details', ["slug" => $product->slug]) }}" class="product-name"><span>{{ $product->name }}</span></a>
                                                <div class="wrap-price"><span class="product-price">{{ number_format($product->regular_price, 0, '', ' ') }} Fcfa</span></div>
                                            </div>
                                        </div>
                                    @endforeach
                                </div>
